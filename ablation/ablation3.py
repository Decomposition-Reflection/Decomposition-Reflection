...
for experiment in range(1):
    ...
    while i<n:
        ...
        output3 = output_7b1(prompt3)
        x = len(prompt3)
        output3 = output3[0][x:-4]
    
        answers1.append(output3)
        answers2.append(merged_output2)
        i += 1
    
    
    with open('results/llama/ablation/expertqa_loseloop3_answers{experiment+2}(0.450.75).json', 'w') as file:
            json.dump(answers1, file)
    with open('results/llama/ablation/QAdatabase/expertqa_loseloop3_answers{experiment+2}(0.450.75)_QAdatabase.json', 'w') as file:
