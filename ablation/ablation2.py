...
def get_top_sublists_and_removed_elements(arrage):
    sublists_and_scores = [(arrage[:i] + arrage[i + 1:], calculate_selfBleu(arrage[:i] + arrage[i + 1:]), arrage[i]) for
                           i in range(len(arrage))]
...
for experiment in range(3):
    ...
    while i<n:
        ...
        while m_loop_key < 5:
            ...
            print("score3,loop-num:", score3, m_loop_key)
            threshold_m = 0.75
            ...
            #message3.append(loop_prompt3)
            prompt33 = prompt33 + '\n' + loop_prompt3
        
        answers1.append(max_output3)
        answers2.append(merged_output2)
        i += 1
    with open('results/llama/ablation/expertqa_loseloop2_answers{experiment+2}(0.450.75).json', 'w') as file:
            json.dump(answers1, file)
    with open('results/llama/ablation/QAdatabase/expertqa_loseloop2_answers{experiment+2}(0.450.75)_QAdatabase.json', 'w') as file:
